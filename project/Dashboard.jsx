...
/* ── Mobile Bottom Nav ── */
function MobileNav({ nav, setNav, user, onLogout }) {
  const isGov = user.role === 'gov' || user.role === 'org';
  const items = isGov
    ? [
    ...
    <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:50,background:'#ffffff',borderTop:'1px solid rgba(0,60,180,0.12)',display:'flex',height:58,boxShadow:'0 -2px 16px rgba(0,60,180,0.08)'}}>
      {items.map(item => (
        <button key={item.id} onClick={() => setNav(item.id)}
          style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:2,border:'none',background:'transparent',cursor:'pointer',fontFamily:'inherit',padding:'6px 0',position:'relative',
            color: nav===item.id ? '#0060e0' : item.accent ? '#059669' : '#6b7fa3'}}>
    ...
    <div style={{flex:1,overflow:'auto',background:'#f4f8ff',width:'100%'}}>
      <div style={{background:'#ffffff',borderBottom:'1px solid rgba(0,60,180,0.1)',padding: mobile?'14px 16px':'28px 32px 22px'}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'nowrap',overflowX:'auto',paddingBottom:2}}>
          {window.HubData.AREAS.slice(0, mobile?5:6).map(a => (
          ...
          {!mobile && window.HubData.INSTITUTIONS.slice(0,3).map(i => (
      ...
      <div style={{padding: mobile?'14px 16px':'24px 32px'}}>
        <div style={{fontSize:12,color:'#6b7fa3',marginBottom:12}}>{filtered.length} pesquisa{filtered.length!==1?'s':''} encontrada{filtered.length!==1?'s':''}</div>
    ...
    <div style={{flex:1,overflow:'auto',background:'#f4f8ff'}}>
      ...
      <div style={{padding: mobile?'16px':'28px 32px',maxWidth: mobile?'100%':880,margin:'0 auto'}}>
        <div style={{marginBottom:16}}>
          ...
          <h1 style={{margin:0,fontSize: mobile?18:24,fontWeight:800,color:'#0d1f3c',lineHeight:1.3,letterSpacing:'-0.3px',textWrap:'pretty'}}>{project.title}</h1>
        </div>
...
Object.assign(window, { Sidebar, MobileNav, Dashboard, ResearchDetail, OdsBadge, Avatar, StatusBadge });
