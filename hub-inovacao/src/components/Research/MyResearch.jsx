      ...
      <div style={{ background: '#ffffff', borderBottom: '1px solid rgba(0,60,180,0.1)', padding: '22px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#6b7fa3' }}>
            {loading ? 'Carregando...' : `${projects.length} projeto${projects.length !== 1 ? 's' : ''} ativo${projects.length !== 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 48, color: '#6b7fa3' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: '#0d1f3c' }}>Nenhuma pesquisa ainda</div>
            <div style={{ fontSize: 12, marginTop: 5 }}>Clique em "+ Nova Pesquisa" para começar</div>
          </div>
        ) : (
          projects.map(p => (
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {p.status === 'published' && (
                  <div style={{ display: 'flex', gap: 16, marginTop: 6 }}>
                    ...
                    <span style={{ fontSize: 12, color: '#059669', fontWeight: 600 }}>🔗 {p.connections_count} conexões</span>
                  </div>
                )}
              </div>
